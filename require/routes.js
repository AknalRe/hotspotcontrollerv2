...
router.post("/testing", async (req, res) => {
  ...
  res.json({
    success: success,
    title: `Testing Toast`,
    message: `Hello Worldaaaaaaaaaaaaaa`,
  });
});
  ...
  try {
    ...
    if (user) {
      ...
      await notif(
        ...
        req.session.role,
        `${ip} - Berhasil login username : ${username}`
      );
      res.json({
        success: true,
        title: `Autentikasi`,
...
router.post("/listakun", isAuthenticated, async (req, res) => {
  ...
  if (response.success) {
    res.json({
      success: true,
      title: `List Akun`,
      message: `Berhasil mendapatkan data list akun`,
      data: response.data,
    });
  } else {
    res.json({
      success: false,
      title: `List Akun`,
      message: `Gagal mendapatkan data list akun`,
      data: "",
    });
  }
});
...
router.post("/exportakun", isAuthenticated, async (req, res) => {
  ...
  if (response.success) {
    const transformedData = response.data.map((item, index) => ({
      ...
      Pemakaian:
        formatkuota(item["bytes-out"]) + " / " + formatkuota(item["bytes-in"]),
    ...
    if (exports.success) {
      await notif(
        ...
        req.session.username,
        req.session.role,
        `Mengeksport list akun nama "${title}"`
      );
    }
    res.json(exports);
  } else {
    res.json({
      success: false,
      title: `Export Akun`,
      message: `Gagal melakukan export akun`,
    });
  }
});
...
router.post("/hapusexport", isAuthenticated, async (req, res) => {
  ...
  if (file) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      res.json({
        success: true,
        title: `Hapus Export Akun`,
        message: `Berhasil menghapus file ${file}`,
      });
    } else {
    ...
    res.json({
      success: false,
      title: `Hapus Export Akun`,
      message: `Gagal melakukan hapus file export akun`,
    });
  }
});
    ...
    if (mikrotikstatus == true) {
      try {
        ...
        await notif(
          ...
          req.session.username,
          req.session.role,
          `Berhasil ${message} akun ${nama}`
        );
        logg(
          true,
          `(${req.session.username}) Berhasil ${message} user (${nama})`
        );
        res.json({
          success: true,
          message: `Berhasil ${message} user (${nama})`,
        });
      } catch (err) {
        logg(
          false,
          `(${req.session.username}) Gagal ${message} user (${nama})`
        );
        res.json({
          success: false,
          message: `Gagal ${message} user (${nama})`,
        });
      }
    } else {
  ...
  if (req.session.role.toLowerCase() !== "demo") {
    ...
    if (mikrotikstatus == true) {
      try {
        ...
        logg(
          true,
          `(${req.session.username}) Berhasil menghapus user (${nama})`
        );
        res.json({
          success: true,
          message: `Berhasil menghapus user (${nama})`,
        });
      } catch (err) {
        logg(false, `(${req.session.username}) Gagal menghapus user (${nama})`);
        res.json({
          success: false,
          message: `Gagal menghapus user (${nama})`,
          response: err,
        });
      }
    } else {
...
router.post("/editakunhotspot", isAuthenticated, async (req, res) => {
  if (req.session.role.toLowerCase() !== "demo") {
    const { usernamelama, id, username, password, jenisAkun, comment } =
      req.body;
    ...
    if (response.success) {
      await notif(
        ...
        req.session.username,
        req.session.role,
        `Berhasil mengubah data akun ${usernamelama} menjadi ${username}-${jenisAkun}`
      );
      res.json(response);
    } else {
      res.json(response);
    ...
    try {
      if (mikrotikstatus) {
        ...
        await notif(
          ...
          req.session.username,
          req.session.role,
          `Berhasil melogout (${nama}) user`
        );
        res.json({
          success: true,
          title: "User Aktif Logout",
          message: `Berhasil melogout (${nama}) user`,
        });
      } else {
      ...
      res.json({
        success: false,
        title: "User Aktif Logout",
        message: `Error melogout (${nama}) user, error: ${err.message}`,
      });
    }
  } else {
    res.json({ success: false, message: `Gagal, Anda berada di user demo` });
    ...
    try {
      if (mikrotikstatus) {
        ...
        await notif(
          ...
          req.session.username,
          req.session.role,
          `Berhasil melogout (${count}) user`
        );
        res.json({
          success: true,
          title: "User Aktif Logout",
          message: `Berhasil melogout (${count}) user`,
        });
      } else {
        logg(false, `Mikrotik tidak terhubung`);
        res.json({
      ...
      res.json({
        success: false,
        title: "User Aktif Logout",
        message: `Error melogout semua user, error: ${err.message}`,
      });
    }
  } else {
    res.json({ success: false, message: `Gagal, Anda berada di user demo` });
    ...
    try {
      if (mikrotikstatus) {
        ...
        await notif(
          ...
          req.session.username,
          req.session.role,
          `Berhasil mengedit limitasi (${nama})`
        );
        res.json({
          success: true,
          title: "User Aktif Logout",
          message: `Berhasil mengedit limitasi (${nama})`,
        });
      } else {
        logg(false, `Mikrotik tidak terhubung`);
        res.json({
      ...
      logg(
        false,
        `Berhasil mengedit limitasi (${nama}), error: ${err.message}`
      );
      res.json({
        success: false,
        title: "User Aktif Logout",
        message: `Berhasil mengedit limitasi (${nama}), error: ${err.message}`,
      });
    }
  } else {
    res.json({ success: false, message: `Gagal, Anda berada di user demo` });
  ...
  try {
    if (mikrotikstatus) {
      ...
      res.json({
        success: true,
        title: "List Binding",
        message: "Berhasil mendapatkan list binding",
        data: filteredData,
      });
    } else {
      res.json({
        success: false,
        title: "List Binding",
        message: "Gagal mendapatkan list binding, mikrotik tidak terhubung",
        data: "",
      });
    }
  } catch (err) {
    res.json({
      success: false,
      title: "List Binding",
      message: `Gagal mendapatkan list binding, terjadi kesalahan : ${err.message}`,
      data: "",
    });
  }
});
    ...
    try {
      if (mikrotikstatus) {
        ...
        await notif(
          ...
          req.session.username,
          req.session.role,
          `Berhasil menghapus binding ${data["mac-address"]}-${data.address}`
        );
        res.json({
          success: true,
          title: "Hapus Binding",
          message: `Berhasil menghapus binding ${data["mac-address"]}-${data.address}`,
          data: "",
        });
      } else {
        res.json({
          success: false,
          title: "Hapus Binding",
          message: "Gagal hapus binding, mikrotik tidak terhubung",
          data: "",
        });
      }
    } catch (err) {
      res.json({
        success: false,
        title: "Hapus Binding",
        message: `Gagal hapus binding, terjadi kesalahan : ${err.message}`,
        data: "",
      });
    }
  } else {
    ...
    try {
      if (mikrotikstatus) {
        ...
        await notif(
          ...
          req.session.username,
          req.session.role,
          `Berhasil ${message} binding ${data["mac-address"]}-${data.address}`
        );
        res.json({
          success: true,
          title: "Binding",
          message: `Berhasil ${message} binding ${data["mac-address"]}-${data.address}`,
          data: "",
        });
      } else {
        res.json({
          success: false,
          title: "Binding",
          message: `Gagal ${message} binding, mikrotik tidak terhubung`,
          data: "",
        });
      }
    } catch (err) {
      res.json({
        success: false,
        title: "Binding",
        message: `Gagal ${message} binding, terjadi kesalahan : ${err.message}`,
        data: "",
      });
    }
  } else {
  ...
  try {
    if (mikrotikstatus) {
      ...
      res.json({
        success: true,
        title: "Log Hotspot",
        message: "Berhasil mendapatkan log hotspot",
        data: loghotspot,
      });
    } else {
      res.json({
        success: false,
        title: "Log Hotspot",
        message: "Gagal mendapatkan log hotspot, mikrotik tidak terhubung",
        data: "",
      });
    }
  } catch (err) {
    res.json({
      success: false,
      title: "Log Hotspot",
      message: `Gagal mendapatkan log hotspot, terjadi kesalahan : ${err.message}`,
      data: "",
    });
  }
});
  ...
  if (mikrotikstatus) {
    try {
      ...
      res.json({
        ...
        message: `Berhasil mendapatkan data banner.js`,
        response: response,
      });
    } catch (err) {
      ...
      res.json({
        success: false,
        title: `Banner Hotspot`,
        message: `Gagal mendapatkan data banner.js`,
    ...
    if (mikrotikstatus) {
      try {
        ...
        await notif(
          ...
          req.session.username,
          req.session.role,
          `Berhasil mengupdate banner ${mikrotikidentity}\n\n${pesan}`
        );
        res.json({
          success: true,
          title: `Ubah Banner Hotspot`,
          message: `Berhasil merubah content banner`,
        });
      } catch (err) {
        ...
        res.json({
          success: false,
          title: `Ubah Banner Hotspot`,
          message: `Gagal merubah content banner`,
          response: err.message,
        });
      }
    } else {
...
router.post("/logoutuser", isAuthenticated, async (req, res) => {
  ...
  const message = id
    ? `melogout user ${id}-${mac}`
    : `melogout semua user ${user}`;
  if (mikrotikstatus) {
    try {
      let aktifuser;
      if (!id) {
...
router.post("/generateqr", isAuthenticated, async (req, res) => {
  ...
  try {
    if (tipe === "link") {
      ...
      await QRCode.toFile(pathfile, link, {
      ...
      return res.json({
        success: true,
        message: `Berhasil generate QR (${nama})`,
        url: `${nama}.png`,
      });
    } else if (tipe === "wifi") {
      wifiqr = password
        ...
        : `WIFI:T:NONE;S:${ssid};;`;
      pathfile = `./views/public/images/${nama}.png`;
      await QRCode.toFile(pathfile, wifiqr, {
        errorCorrectionLevel: "H",
        scale: 8,
      });
      logg(true, `Berhasil generate QR (${nama})`);
    ...
    return res.json({
      success: false,
      title: `Generate Qr`,
      message: `Gagal Generate QR (${nama}), error: ${error.message}`,
    });
  }
});
...
router.post("/hapusgenerateqr", isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    if (fs.existsSync(filepath)) {
      ...
      res.json({
        success: true,
        title: `Hapus Images QR`,
        message: `Images QR ${file} berhasil dihapus`,
      });
    } else {
      ...
      res.json({
        success: false,
        title: `Hapus Images QR`,
        message: `Images QR ${file} tidak ditemukan`,
      });
    }
  } catch (err) {
    ...
    res.json({
      success: false,
      title: `Hapus Images QR`,
      message: `Gagal menghapus Images QR ${file}, error: ${err.message}`,
    });
  }
});
...
router.post("/listklienbroadcast", isAuthenticated, async (req, res) => {
  ...
  if (mikrotikstatus) {
    try {
      ...
      res.json({
        success: true,
        title: `List Klien Broadcast`,
        message: `Berhasil mendapatkan list klien broadcast`,
        data: sendData,
      });
    } catch (er) {
      logg(
        false,
        `Gagal mendapatkan list klien broadcast, error: ${er.message}`
      );
      res.json({
        success: false,
        title: `List Klien Broadcast`,
        message: `Gagal mendapatkan list klien broadcast, error: ${er.message}`,
      });
    }
  } else {
    logg(false, `Mikrotik tidak terhubung`);
    ...
    try {
      if (Array.isArray(tujuan)) {
        ...
        for (let i = 0; i < tujuanlength; i++) {
          ...
          results.push({ tujuan: tujuan[i], msg: result });
          jumlahkirim++;
          if (jumlahkirim === 5) {
            ...
            await new Promise((resolve) =>
              setTimeout(resolve, nilaiRandom * 1000)
            ); // Menunggu 2 detik
          }
        }
      } else {
        ...
        results.push({ tujuan: tujuan, msg: result });
      }

      res.json({
        success: true,
        title: `Broadcast Klien`,
        message: `Broadcast berhasil memproses ${message}`,
        result: results,
      });
    } catch (err) {
      res.json({
        success: false,
        title: `Broadcast Klien`,
        message: `Gagal broadcast ke klien, error: ${err.message}`,
      });
    }
  } else {
    res.json({
...
router.post("/updateservjs", isAuthenticated, async (req, res) => {
  if (req.session.role.toLowerCase() !== "demo") {
      ...
      if (mikrotikstatus) {
          try {
              if (!contents) {
                  return res.json({ success: false, message: `Gagal merubah content Sistem` });
              }
...
router.use(async (req, res) => {
  ...
  res
    .status(404)
    .send(
      '<html><head><link rel="icon" type="image/x-icon" href="https://merch.mikrotik.com/cdn/shop/files/512.png"><title>404</title></head><body></body></html>'
    );
});
